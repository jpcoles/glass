...
def report(env):
    ...
    if env.nu is not None: Log( pp('H0inv        = %s'   % str_range(convert('nu to H0^-1 in Gyr',env.nu), '%.4g'), '[Gyr]') )
    if env.nu is not None: Log( pp('H0           = %s'   % str_range(convert('nu to H0 in km/s/Mpc',env.nu), '%.4g'), '[km/s/Mpc]') )
