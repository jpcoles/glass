...
#           Log( "SKIPPING NEIGHBOR FINDING!" )
        ...
        #
        # Convert int_ploc to [-0.5,0.5] x [-0.5,0.5]
        #
        #self.ploc_t = ((self.int_ploc / self.pixrad) + complex(1,1)) / 2 - complex(0.5, 0.5)
        self.ploc_t = ((self.int_ploc / (self.pixrad+1)) + complex(1,1)) / 2 - complex(0.5, 0.5)


        def RBF(N,n,m,t):
            r = abs(t - np.conj(complex(n/N, m/N) - complex(0.5,0.5)))
            eps = 15
            f =  -(2*abs(t))**2 + 1
            z = np.exp(-(eps * r)**2) * f
            return z
        ...
        if 1:
            self.bern_nm = [ 
                [_i,_n,_m, RBF(self.poly_order, _n,_m, self.ploc_t)] 
                for _i,[_n,_m] in enumerate(product(range(self.poly_order+1), repeat=2))
                if abs(np.conj(complex(_n/self.poly_order, _m/self.poly_order) - complex(0.5,0.5))) <= 0.52
            ]

            _ip = [ complex(_n,_m) for [_i,_n,_m,_g] in self.bern_nm ]
            _ip = np.array(_ip) - complex(self.poly_order//2, self.poly_order//2)
            _ip = np.conj(_ip)
            self.oppose_t = [ argwhere(_ip == -l).ravel() for l in _ip ]
...
#           sp.add_artist(Rectangle([r.real-s/2, r.imag-s/2], s,s, fill=False))
        ...
        if obj.symm:
            self.symmetric = True
            self.nvar_symm -= (len(self.bern_nm)-1) // 2
