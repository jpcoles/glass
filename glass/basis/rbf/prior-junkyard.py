...
def lens_eq(o, leq, eq, geq):
    ...
    for i,src in enumerate(o.sources):
        for j,img in enumerate(src.images):
            ...
            for ri, [_n,_m] in enumerate(b.bern_nm, bcoeff_start):
                rows[0,ri] = -np.sum(p_dx * b.gn[_n] * b.gm[_m])
                rows[1,ri] = -np.sum(p_dy * b.gn[_n] * b.gm[_m])
...
#               sm = sm * sol[sm_err]
            ...
            for ri, [_n,_m] in enumerate(b.bern_nm, bcoeff_start):
                r0 -= sol[ri] * np.sum(b.gn[_n] * b.gm[_m] * p_dx)
                r1 -= sol[ri] * np.sum(b.gn[_n] * b.gm[_m] * p_dy)
    ...
    for src in o.sources:
        for img in src.images:
            ...
            if parity == MINIMUM:
                ...
                for ri, [_n,_m] in enumerate(b.bern_nm, bcoeff_start):
                    rows[0,ri] = np.sum((-k1*xx + yy         ) * b.gn[_n] * b.gm[_m])
                    rows[1,ri] = np.sum((-k2*yy + xx         ) * b.gn[_n] * b.gm[_m])
                    rows[2,ri] = np.sum((         xy + xx*eps) * b.gn[_n] * b.gm[_m])
                    rows[3,ri] = np.sum((        -xy + xx*eps) * b.gn[_n] * b.gm[_m])
                    rows[4,ri] = np.sum((         xy + yy*eps) * b.gn[_n] * b.gm[_m])
                    rows[5,ri] = np.sum((        -xy + yy*eps) * b.gn[_n] * b.gm[_m])
...
def profile_steepness(o, leq, eq, geq):
    ...
    for l,[r0,r1] in enumerate(zip(o.basis.rings[:-1], o.basis.rings[1:])):
        ...
        #or (o.basis.hires_levels and l < (o.basis.hiresR*o.basis.hires_levels - o.basis.hires_levels//2)):
            ...
            for ri, [_n,_m] in enumerate(b.bern_nm, bcoeff_start):
                row[ri]  = np.sum((w0 * R1**(-minsteep)) * b.gn[_n][r0] * b.gm[_m][r0])
                row[ri] -= np.sum((w1 * R0**(-minsteep)) * b.gn[_n][r1] * b.gm[_m][r1])
            ...
            if minsteep == maxsteep:
                ...
                if l > 0 and maxsteep is not None:
                    ...
                    for ri, [_n,_m] in enumerate(b.bern_nm, bcoeff_start):
                        row[ri]  = np.sum((w0 * R1**(-maxsteep)) * b.gn[_n][r0] * b.gm[_m][r0])
                        row[ri] -= np.sum((w1 * R0**(-maxsteep)) * b.gn[_n][r1] * b.gm[_m][r1])
                    leq(row)
                    c += 1
            c += 1
...
#       else:
        if 1:
            ...
            for i in ([1,-1] if sn != 0 else [1]):
                ...
                for ri, [_n,_m] in enumerate(b.bern_nm, bcoeff_start):
                    row[ri]  = np.sum(( wght(w) * x) * b.gn[_n][w] * b.gm[_m][w])
                    row[ri] += np.sum((-wght(e) * x) * b.gn[_n][e] * b.gm[_m][e])
                    row[ri] += np.sum(( wght(s) * y) * b.gn[_n][s] * b.gm[_m][s])
                    row[ri] += np.sum((-wght(n) * y) * b.gn[_n][n] * b.gm[_m][n])
...
def check_link_sources(o, sol):
    b = o.basis
    srcpos_start, srcpos_end = 0+b.offs_srcpos
