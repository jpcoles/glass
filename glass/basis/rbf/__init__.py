...
def symm_fold_t(o, row):

    bcoeff_start, bcoeff_end = 1+o.basis.offs_bcoeff

    symm_row = zeros(1+o.basis.nvar_symm)
    symm_row[:bcoeff_start] = row[:bcoeff_start]

    done = zeros(len(o.basis.oppose_t))
    c = bcoeff_start
    for i,j in enumerate(o.basis.oppose_t):
        if i == j:
            symm_row[c] = row[bcoeff_start + i]
            done[i] = 1
            c += 1

        if done[i]: continue

        #j = o.basis.oppose_t[i]
        #print(i,j)
        #j = argwhere(o.basis.int_ploc == -ri).flatten()
        #assert j.size == 1
        #j = j[0]
        ...
        #print(symm_row[c])
        #print(row[bcoeff_start + i])
        #print(row[bcoeff_start + j])
        symm_row[c] = row[bcoeff_start + i] + row[bcoeff_start + j]
        c += 1
    symm_row[c:] = row[bcoeff_end:]
...
def symm_unfold_t(o, symm_row):
    bcoeff_start, bcoeff_end = 0+o.basis.offs_bcoeff

    row = zeros(o.basis.nvar)
    row[:bcoeff_start] = symm_row[:bcoeff_start]

    done = zeros(len(o.basis.oppose_t))
    c = bcoeff_start
    for i,j in enumerate(o.basis.oppose_t):
        if i == j:
            row[bcoeff_start + i] = symm_row[c]
            done[i] = 1
            c += 1
        ...
        row[bcoeff_start + i] = row[bcoeff_start + j] = symm_row[c]
        c += 1

    row[bcoeff_end:] = symm_row[c:]
...
def init_model_generator(env, nmodels, regenerate=False):
    ...
    section( 'RADIAL BASIS FUNCTION MODEL GENERATOR' )
#   if nmodels == 0:
