...
#           Log( "SKIPPING NEIGHBOR FINDING!" )
        ...
        #
        # Convert int_ploc to [0,1] x [0,1]
        #
        #self.ploc_t = ((self.int_ploc / self.pixrad) + complex(1,1)) / 2
        self.ploc_t = ((self.int_ploc / (self.pixrad+1)) + complex(1,1)) / 2

        def B(n,k,t):
            assert np.all(0 <= t) and np.all(t <= 1)
            return ( binom(n,k) * ((1-t)**(n-k) * (t**k)) )

        self.gn = [ B(self.poly_order, k, self.ploc_t.real) for k in range(self.poly_order+1) ] 
        self.gm = [ B(self.poly_order, k, self.ploc_t.imag) for k in range(self.poly_order+1) ] 

        def B2(n,m,k,t):
            #u,v = t.real, t.imag
            #assert np.all(0 <= t) and np.all(t <= 1)
            #t1 =  0.5 * (np.sqrt(2 + u**2 - v**2 + 2*u*np.sqrt(2)) - np.sqrt(2 + u**2 - v**2 - 2*u*np.sqrt(2)))
            #t2 =  0.5 * (np.sqrt(2 - u**2 + v**2 + 2*v*np.sqrt(2)) - np.sqrt(2 - u**2 + v**2 - 2*v*np.sqrt(2)))
            if 0:
                x,y = t.real-0.5, t.imag-0.5
                t1 = x * np.sqrt(1 - 0.5*y**2) + 0.5
                t2 = y * np.sqrt(1 - 0.5*x**2) + 0.5
            else:
                t1 = abs(t.real-0.5) if m in [0,n] else t.real 
                t2 = abs(t.imag-0.5) if k in [0,n] else t.imag 
            return ( (binom(n,m) * ((1-t1)**(n-m) * (t1**m))) 
                   * (binom(n,k) * ((1-t2)**(n-k) * (t2**k))) ) 

        def B3(n,m,k,t):
            if m==n//2 and k==n//2:
                #print( (abs(t-complex(0.5,0.5)) + 0.01) ** (-1.0)) / 
                #sys.exit(0)
                return np.exp(-(abs(t-complex(0.5,0.5)) + 0.01) / 0.001) - 3
            #return (abs(t-complex(0.5,0.5)) + 0.01) ** (-1)
                #r = np.sqrt((t-0.5)**2 + 0.001)
                #r0 = np.sqrt((0-0.5)**2 + 0.001)
                #exp(-r/.2) - np.exp(-r0/.2)
            #z = np.zeros(len(t))
            #    z[n//2] = 1
            #    return z
                ...
                x = t.real 
                y = t.imag 
                return ( (binom(n, m) * x**m * (1 - x)**(n - m)) *
                         (binom(n, k) * y**k * (1 - y)**(n - k))
                       )

        self.gs = []
        for _n,_m in product(range(self.poly_order+1), repeat=2):
            g = B2(self.poly_order, _n,_m, self.ploc_t)
            self.gs.append(g)


        if 0:
            #self.bern_nm = list(product(range(self.poly_order+1), repeat=2))
            self.bern_nm = [ 
                [_i,_n,_m, B3(self.poly_order, _n,_m, self.ploc_t)] 
                for _i,[_n,_m] in enumerate(product(range(self.poly_order+1), repeat=2))
            ]
        elif 0:
            self.bern_nm = [ 
                [_i,_n,_m, B2(self.poly_order, self.poly_order-_n,self.poly_order-_m, self.ploc_t)] 
                for _i,[_n,_m] in enumerate(product(range(self.poly_order+1), repeat=2))
                if abs(complex(_n/self.poly_order, _m/self.poly_order) - complex(0.5,0.5)) <= 0.51
            ]
        elif 1:
            self.bern_nm = [ 
                [_i,_n,_m, B2(self.poly_order, _n,_m, self.ploc_t)] 
                for _i,[_n,_m] in enumerate(product(range(self.poly_order+1), repeat=2))
                if  0 < _n < self.poly_order and 0 < _m < self.poly_order
            ]
        else:
            self.bern_nm = [ 
                [_i,_n,_m, B3(self.poly_order, _n,_m, self.ploc_t)] 
                for _i,[_n,_m] in enumerate(product(range(self.poly_order+1), repeat=2))
                if  (0 < _n < self.poly_order and 0 < _m < self.poly_order) or (n==0 and m==0)
            ]

        if 0:
            M = np.zeros([self.poly_order+1,self.poly_order+1])
            for _n,_m in product(range(self.poly_order+1), repeat=2):
                if abs(complex(_n/self.poly_order,_m/self.poly_order)-complex(0.5,0.5)) <= 0.51:
                    M[_n,_m] = 1
            pl.imshow(M, extent=[0,1,0,1])
            pl.savefig('M.pdf')
            #pl.show()
            sys.exit(0)
...
#           sp.add_artist(Rectangle([r.real-s/2, r.imag-s/2], s,s, fill=False))
        ...
        last,self.offs_bcoeff = [last+len(self.bern_nm), 
                                 last+array([0, len(self.bern_nm)], dtype='int32')]
    ...
    def report(self):
        ...
        Log( ii+'        bcoeff     % 5i  % 5i' % (self.offs_bcoeff[0], self.offs_bcoeff[1]) )
...
#       sys.exit(0)
    ...
    def solution_to_dict(self, sol):
        ...
        ps['bcoeffs']         = sol[self.offs_bcoeff[0]:self.offs_bcoeff[1]]


        k = np.zeros(len(self.ploc_t))
        #for i in range(len(k)):
        for ri, [_i,_n,_m,_g] in enumerate(self.bern_nm):
            k += ps['bcoeffs'][ri] * _g
            #k += ps['bcoeffs'][ri] * self.gn[_n] * self.gm[_m]

        #for i in range(len(k)):
        #    for ri, [_n,_m] in enumerate(self.bern_nm, self.offs_bcoeff[0]):
        #        k[i] += self.gn[_n][i] * self.gm[_m][i] * sol[ri]
